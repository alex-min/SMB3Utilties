...
#ifdef LSB_FIRST
#define nes_pixel(surface_data, surface_offset, pal, alpha) \
	{                                                       \
		surface_data[surface_offset++] = (pal)->b;          \
		surface_data[surface_offset++] = (pal)->g;          \
		surface_data[surface_offset++] = (pal)->r;          \
		surface_data[surface_offset++] = (alpha);           \
	}
#else
#define nes_pixel(surface_data, surface_offset, pal, alpha) \
	{                                                       \
...
static void ppu_init_objs(struct _PPU_SPR *ppu_spr_base, int is_map_objects)
{
	...
	for (i = 0; i < 256; i++)
	{
		struct NoDice_objects *this_obj = (!is_map_objects) ? &NoDice_config.game.regular_objects[i] : &NoDice_config.game.map_objects[i];
